...
public class GroupControllerTest {
    ...
    public void createGroup() throws Exception {
        mockMvc.perform(post("/groups").contentType(MediaType.APPLICATION_PROTOBUF_VALUE).content(groupModel.toByteArray())).andDo(print()).andExpect(status().isOk());
    ...
    public void getGroupById() throws Exception {
        ...
        var result = mockMvc.perform(get("/groups?id=1")).andDo(print()).andExpect(status().isOk()).andReturn();

        Assertions.assertEquals(MediaType.APPLICATION_PROTOBUF_VALUE, result.getResponse().getContentType());
        GroupModel responseModel = GroupModel.parseFrom(result.getResponse().getContentAsByteArray());
        Assertions.assertEquals(responseModel, groupModel);
    }
